...
async def test_non_admin_cannot_query_other_users_logs(client, db_session):
    """Non-admin users should only see their own audit logs, even if they
    pass another user_id as a query parameter."""
    token = await _register_and_get_token(client)
    ...
    # Demote user to installer (non-admin)
    await db_session.execute(
        update(User).where(User.id == user_id).values(role=Role.INSTALLER)
    )
    await db_session.commit()
    ...
    # Try to query with the other user's ID — should be ignored and scoped
    # to the current user
    resp = await client.get(
        f"/api/audit-logs?user_id={other_user_id}",
        headers={"Authorization": f"Bearer {token}"},
    )
    assert resp.status_code == 200
    data = resp.json()
    # Should only see their own log, not the other user's
    assert data["total"] == 1

...
const statusStyles: Record<string, { bg: string; text: string; label: string }> = {
  pending: { bg: 'bg-[var(--surface-app)]', text: 'text-[var(--text-secondary)]', label: 'Pending' },
      ...
      <div className="shrink-0 border-b border-[var(--border)] bg-[var(--surface-card)] px-6 py-4">
        ...
        <div className="mt-3 flex gap-2">
          {Array.from({ length: 4 }).map((_, i) => (
            <div key={i} className="h-8 w-20 animate-pulse rounded-lg bg-[var(--surface-app)]" />
          ))}
        </div>
      </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          {Array.from({ length: 12 }).map((_, i) => (
            <div key={i} className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-card)]">
              <div className="h-40 w-full animate-pulse bg-[var(--surface-app)]" />
              <div className="p-4 space-y-2">
                <div className="h-4 w-3/4 animate-pulse rounded bg-[var(--surface-raised)]" />
                <div className="h-3 w-1/2 animate-pulse rounded bg-[var(--surface-app)]" />
                <div className="h-3 w-1/3 animate-pulse rounded bg-[var(--surface-app)]" />
              </div>
            </div>
          ))}
        </div>
...
export default function ThreeDPage() {
  ...
  return (
    <div className="flex h-full flex-col">
      {/* Toast */}
      {toast && (
        <div className="fixed bottom-6 right-6 z-50 rounded-lg bg-gradient-to-r from-[var(--accent-primary)] to-[var(--accent-secondary)] px-4 py-3 text-sm text-white shadow-lg">
          {toast}
        </div>
      )}
      ...
      <header className="shrink-0 border-b border-[var(--border)] bg-[var(--surface-card)] px-6 py-4">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {/* View toggle */}
            <div className="flex rounded-lg border border-[var(--border)] bg-[var(--surface-app)] p-0.5">
              <button
          ...
          {filterTabs.map(({ key, label }) => (
            <button
              ...
              className={`rounded-lg px-3 py-1.5 text-xs font-medium transition-colors ${
                filter === key ? 'bg-blue-50 text-blue-700' : 'text-[var(--text-secondary)] hover:bg-[var(--surface-overlay)]'
              }`}
            >
              {label}
            </button>
          ...
          <div className="grid grid-cols-3 gap-4">
            {renders.map((r) => {
              ...
              return (
                <div
                  key={r.id}
                  className="overflow-hidden rounded-xl border border-[var(--border)] bg-[var(--surface-card)] transition-colors hover:border-blue-200"
                >
                  {/* Thumbnail */}
                  {r.result_image_url ? (
                    <img
                    ...
                    <div className="flex h-40 items-center justify-center bg-gradient-to-br from-[var(--surface-app)] to-[var(--border)]">
                    ...
                    <div className="mt-3 flex items-center gap-2">
                      {r.status === 'completed' && (
                        <button
                          ...
                          className="rounded-md border border-[var(--border)] px-2.5 py-1 text-[11px] font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-overlay)]"
                        >
                          Share
                        </button>
                      )}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {renders.map((r) => {
                  const style = statusStyles[r.status] ?? statusStyles.pending;
                  return (
                    <tr key={r.id} className="border-b border-[var(--border)] last:border-0 hover:bg-[var(--surface-app)]/50">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-3">
                          {r.result_image_url ? (
                            ...
                            <div className="flex h-10 w-14 shrink-0 items-center justify-center rounded bg-gradient-to-br from-[var(--surface-app)] to-[var(--border)]">
          ...
          <div className="mt-4 flex items-center justify-between">
            ...
            <div className="flex gap-1">
              <button
                ...
                className="rounded-lg px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-overlay)] disabled:opacity-40"
              >
                Previous
              </button>
              <button

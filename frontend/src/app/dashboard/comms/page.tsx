        ...
        <div className="w-80 shrink-0 border-r border-[var(--border)] bg-[var(--surface-card)]">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="flex items-start gap-3 border-b border-[var(--border)] px-4 py-3">
              <div className="h-9 w-9 shrink-0 animate-pulse rounded-full bg-[var(--surface-raised)]" />
              <div className="flex-1 space-y-2">
                <div className="h-4 w-28 animate-pulse rounded bg-[var(--surface-raised)]" />
                <div className="h-3 w-full animate-pulse rounded bg-[var(--surface-app)]" />
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="space-y-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className={`flex ${i % 2 === 0 ? 'justify-start' : 'justify-end'}`}>
                <div className="h-16 w-72 animate-pulse rounded-xl bg-[var(--surface-app)]" />
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="flex items-center justify-between">
          ...
          {unreadCount > 0 && (
            <button
              ...
              className="text-xs font-medium text-[var(--accent-primary)] hover:text-blue-700 disabled:opacity-50"
            >
                ...
                >
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-[10px] text-[var(--text-muted)]">{formatTimestamp(n.created_at)}</span>
                    </div>
                    <p className="mt-0.5 truncate text-xs text-[var(--text-secondary)]">{n.body}</p>
                  </div>
                  {!n.is_read && (
          ...
          <div className="flex-1 overflow-y-auto p-6">
            {selectedNotification ? (
              <div className="mx-auto max-w-2xl">
                ...
                <div className="rounded-xl bg-[var(--surface-app)] px-5 py-4">
              ...
              <div className="flex flex-wrap gap-2">
                {templates.map((t) => (
                  <button
                    ...
                    className="rounded-lg border border-[var(--border)] px-3 py-1.5 text-xs font-medium text-[var(--text-secondary)] transition-colors hover:bg-[var(--surface-overlay)] disabled:opacity-50"
                    title={t.subject}
                  >

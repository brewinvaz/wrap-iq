...
export default function InsightsTable({ data }: InsightsTableProps) {
  ...
  const sortIcon = (col: SortKey) => {
    if (sortKey !== col) return <span className="ml-1 text-[var(--text-muted)]">{'\u2195'}</span>;
  ...
  return (
    <div className="overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--surface-card)]">
      <div className="px-5 py-4 border-b border-[var(--border)]">
        <h3 className="text-[15px] font-semibold text-[var(--text-primary)]">Installer Performance</h3>
      </div>
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b border-[var(--border)] bg-[var(--surface-raised)]">
            <th
              className="sticky top-0 cursor-pointer px-5 py-2.5 text-left font-mono text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]"
            ...
            <th className="sticky top-0 px-5 py-2.5 text-left font-mono text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]">
              Performance
            </th>
            <th
              className="sticky top-0 cursor-pointer px-5 py-2.5 text-left font-mono text-[10px] font-bold uppercase tracking-wider text-[var(--text-muted)]"
          ...
          {sorted.map((installer, i) => (
            <tr
              ...
              className={`border-b border-[var(--border)] last:border-b-0 ${
                i % 2 === 1 ? 'bg-[var(--surface-raised)]/50' : ''
              }`}
            >
              <td className="px-5 py-3">
                <div className="flex items-center gap-2.5">
                  ...
                  <span className="font-medium text-[var(--text-primary)]">{installer.name}</span>
                </div>
              </td>
              <td className="px-5 py-3 font-mono font-semibold text-[var(--text-primary)]">{installer.installs}</td>
              <td className="px-5 py-3">
                <div className="h-2 w-24 overflow-hidden rounded-full bg-[var(--surface-raised)]">
                  <div
              ...
              <td className="px-5 py-3 text-[var(--text-secondary)]">{installer.avgTime}</td>
              <td className="px-5 py-3">
